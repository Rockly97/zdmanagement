...
(function() {
	...
	// Figure out if we'admin running the tests from a server or not
...
Test.prototype = {
	before: function() {
		if (
			...
			// Emit moduleStart when we'admin switching from one module to another
	...
	run: function() {
		...
		try {
			...
			// Restart the tests if they'admin blocking
			if ( config.blocking ) {
				QUnit.start();
			}
		}
